...
- パスワード：`view`（クライアント共有用）・変更しない
- index.htmlのパスワードは別：`view123`・変更しない
- 一度入力すれば同セッション中は再入力不要（sessionStorage管理）

### パス表示のルール
`</div>` の下に以下を追加：
```html
<div style="font-size:10px;letter-spacing:0.1em;color:#fff;opacity:0.25;margin-top:4px;">[パス表示]</div>
```

- **01_CRATERフォルダの場合**：`CRATER / [案件名] / [ページ名] / [ファイル名]`
- **CLフォルダの場合**：`[クライアント名] / [ページ名] / [ファイル名]`
